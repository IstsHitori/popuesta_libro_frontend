...
import type { EarnedGarment, EarnedCrystal, LevelReward } from "../types/earned-items.types";
...
export const useEarnedItemsStore = create<EarnedItemsStore & EarnedItemsActions>()(
  persist(
    (set, get) => ({
      // Estado inicial
      ...
      shouldShowModal: (level: number) => {
        const state = get();
        return state.completedLevels.includes(level) && !state.shownModals.includes(level);
      },
      ...
      resetProgress: () => {
        set({
          garments: LEVEL_REWARDS.map(reward => ({ ...reward.garment, isEarned: false })),
          crystals: LEVEL_REWARDS.map(reward => ({ ...reward.crystal, isEarned: false })),
          completedLevels: [],
          shownModals: [],
        });
      },
