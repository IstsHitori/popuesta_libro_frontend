...
export function useCircuitGame() {
  ...
  const handleComponentDrop = useCallback((
    ...
    // Check if we're dragging FROM a slot (for removal/replacement)
    const isFromSlot = gameState.circuitBoard.inputSlots.some(
      slot => slot.currentComponent?.id === componentId
    );
    
    if (isFromSlot) {
      // If not dropping on a valid slot, just remove component
      const targetSlot = gameState.circuitBoard.inputSlots.find(slot => slot.id === targetSlotId);
      if (!targetSlot) {
        return handleComponentRemoval(componentId);
      }
      // Handle replacement: remove from old slot, then place in new
      const removalResult = handleComponentRemoval(componentId);
      if (removalResult.success) {
        // Find the component in available pool and place it
  ...
  }, [gameState, handleComponentRemoval, handleComponentPlacement]);

  // Check if the circuit is correctly completed
  const checkCircuitCompletion = (state: CircuitGameState): CircuitDragResult => {
